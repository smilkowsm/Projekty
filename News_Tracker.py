# SKRYPT SZUKAJACYCH NEWSOW PO KEYWORDACH
# NIE ZAPISUJE DO PLIKU >TXT
"""***************POBIERANIE INFO ZE STRONY I ZAPIS DO PLIKU plik.txt**************************"""
import re
import requests

# RECZNE WYBIERANIE ZAKREDSU STRON
...
# EUROGAMER
for i in range(zakres_od, zakres_do + 1):
    x = requests.get(f"https://www.eurogamer.pl/news?page={i}") # Get info from webpage
    output = re.sub(r" ", "", x.text)
...
if zakres_od == 1: # gram.pl news start with page index 0, not 1
    for i in range(0, zakres_do + 1): 
        x = requests.get(f"https://www.gram.pl/news?page={i}") # Get info from webpage
